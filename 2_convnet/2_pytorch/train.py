...
if args.model == 'softmax':
    ...
    model = models.mymodel.MyModel(im_size, [3,32,64,128, 256, 256], [5, 5, 3, 3, 3], n_classes)
else:
...
def evaluate(split, verbose=False, n_batches=None):
    ...
    if split == 'val':
        ...
        loader = test_loader
        best_model = torch.load(args.model + '.pt')
    ...
    if acc>best_acc and split=='val':
        print('New model saved with accuracy', acc)
        best_acc=acc
